...
CREATE TABLE orders (
  ...
  -- APONTA PARA A IDENTIDADE UNICA, e nao para uma tabela de clientes (RN-52): o
  ...
  -- ENUMERACAO, E NAO CHAVE ESTRANGEIRA (RN-49). Canal de venda e lista fechada de
  ...
  -- RN-56: tres situacoes, e o que o negocio precisa saber e em qual delas o pedido
...
CREATE TABLE order_items (
  ...
  -- O PRECO E DIGITADO, e o sistema nao o calcula (RF-67, RN-58). Nao ha
...
-- (RF-68) e somado dos lotes prontos daquela especie e recipiente a cada consulta.
...
-- do lote ser visao e nao coluna (RN-31).

COMMENT ON TABLE orders IS
  'Pedido. Tres situacoes; confirmado nao aceita alteracao de item. RF-69, RN-56.';
COMMENT ON COLUMN orders.customer_id IS
  'Pessoa do cadastro unico que exerce o papel de cliente. RN-52.';
COMMENT ON COLUMN order_items.unit_price IS
  'Preco unitario informado por quem registra. O sistema nao o calcula. RN-58.';
