import { type UnidadeTarefa, isUnidadeTarefa } from './agenda-rotulos';
...
export const DECLARACAO_LABELS: Record<keyof Declaracoes, string> = {
  ...
  exigeArea: 'Registra área e canteiro',
};

/**
 * RF-21. O lote já determina a espécie e o canteiro: com lote exigido, pedir um
 * deles seria perguntar o que o sistema já sabe, e abrir espaço para discordarem.
 */
export function parseTipoTarefaFields(input: {
  ...
  if (!isUnidadeTarefa(input.unidadeMedida)) return { error: 'Escolha a unidade da quantidade.' };
  return {
    value: {
      nome,
      ...
      exigeArea: input.exigeLote ? false : input.exigeArea,
...
export function resumoDeclaracoes(tipo: Declaracoes & { unidadeMedida: UnidadeTarefa }): string {
  const pede = [
    tipo.exigeLote && 'lote',
    tipo.exigeEspecie && 'espécie',
    tipo.exigeRecipiente && 'recipiente',
    tipo.exigeArea && 'área e canteiro',
    tipo.eQuantitativa && `quantidade por pessoa em ${tipo.unidadeMedida}`,
  ].filter(Boolean);
...
const SELECT_TIPO = `
  SELECT id, nome, categoria, e_quantitativa AS "eQuantitativa", exige_lote AS "exigeLote",
         exige_especie AS "exigeEspecie", exige_recipiente AS "exigeRecipiente", exige_area AS "exigeArea",
         unidade_medida AS "unidadeMedida", ativo
    FROM tipos_tarefa`;
...
export async function insertTipoTarefa(db: Db, input: TipoTarefaFields): Promise<string> {
  const { rows } = await db.query<{ id: string }>(
    `INSERT INTO tipos_tarefa (nome, categoria, e_quantitativa, exige_lote, exige_especie, exige_recipiente, exige_area, unidade_medida)
  ...
  const { rowCount } = await db.query(
    `UPDATE tipos_tarefa
        SET nome = $2, categoria = $3, e_quantitativa = $4, exige_lote = $5, exige_especie = $6,
            exige_recipiente = $7, exige_area = $8, unidade_medida = $9, ativo = $10
      WHERE id = $1`,
    [
      id,
      changes.nome,
