...
|---|---|
| **Aplicação web progressiva** | Atende ao uso móvel (RE-2) sem exigir instalação por loja de aplicativos (RNF-23), o que elimina custo e fricção de distribuição para nove usuários |
...
| Banco relacional com integridade declarativa | Alterações de esquema exigem migração versionada (RNF-17); em troca, o dado inconsistente é impedido pelo banco e não apenas pela aplicação |
...
| RNF-17: esquema versionado | Migrações aplicadas de forma controlada na publicação (ver [`D3`](D3-diagrama-implantacao.md)) |
