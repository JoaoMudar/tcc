...
### 4.7.1.1 Nota sobre o administrador na implementação
...
## 4.7.2 Matriz por recurso e operação
...
[`B2 §4.7.2`](../B-requisitos/B2-especificacao-requisitos.md), onde esta matriz vira código.

**A permissão é do recurso, não da área.** Nenhuma linha abaixo é concedida por a tela ficar sob
`/producao` ou `/comercial`: a guarda é por recurso, em toda operação (§4.7.4). O agrupamento serve
...
¹ **Ninguém cria e ninguém exclui parâmetro.** Ver §4.7.3.7.
...
acesso que não se leem direto da matriz estão em §4.7.3.8.
...
sem nunca alterar quantidade, preço ou item, que continuam `L` para ela. Ver §4.7.3.2.
...
### 4.7.3.1 A gerência não vê dado fiscal de pessoa
...
### 4.7.3.2 A gerência executa duas fases do pedido, e não cadastra nenhum
...
juntas, e as duas rodam no servidor (§4.7.4).
...
colaboradores não abrem tela (§4.7.1), e a decisão continua valendo aqui.
...
### 4.7.3.3 A chefia não monta a agenda
...
### 4.7.3.4 A gerência não cadastra espécie, recipiente nem insumo
...
### 4.7.3.5 Ninguém administra os próprios usuários
...
### 4.7.3.6 A gerência cria lote e movimento, mas não exclui
...
### 4.7.3.7 Configurações: ninguém cria e ninguém exclui
...
### 4.7.3.8 O protocolo é da gerência tanto quanto da chefia, e a divisão é só da gerência
...
### 4.7.3.9 Nota sobre o alcance da matriz
...
## 4.7.4 Onde a verificação ocorre
...
### 4.7.4.1 Por que não há um quarto nível no banco
...
## 4.7.5 O compromisso entre proteção e produtividade
...
## 4.7.9 Síntese pelos três tipos de ameaça
...
## 4.7.10 Síntese pelos três tipos de controle
...
## 4.7.11 O que este sistema deliberadamente não faz
...
## 4.7.13 Por que este artefato existe
...
## 4.7.14 Inventário do tratamento

### 4.7.14.1 Dados de clientes
...
### 4.7.14.2 Dados de fornecedores
...
### 4.7.14.3 Dados de usuários do sistema
...
### 4.7.14.4 Dados de funcionários que não usam o sistema: **o caso mais sensível**
...
## 4.7.15 Direitos dos titulares e como o sistema os atende
...
2. **Não há aviso de privacidade apresentado aos titulares.** Clientes e fornecedores não são
   ...
   grave, porque não há tela pela qual informá-los (§4.7.14.4).
...
## 4.7.16 Síntese das bases legais
...
## 4.7.17 O que se perde, e por que importa
...
### 4.7.19.1 Cópia automatizada do banco
...
### 4.7.19.4 O que não é copiado, e por quê
...
## 4.7.20 Procedimento de recuperação
...
## 4.7.22 Relação com os riscos e ameaças
