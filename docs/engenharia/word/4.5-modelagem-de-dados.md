...
([`B2 §4.5.2`](../B-requisitos/B2-especificacao-requisitos.md)) e da matriz de acesso
([`D4 §4.5.2`](../D-arquitetura/D4-matriz-rbac.md)) permite ler os três documentos lado a lado sem
...
`turnos_trabalho`, que são do Cadastro único. A convenção do §4.5.3 resolve isso sem duplicar conteúdo: a
...
## 4.5.2 Modelo conceitual: visão geral
...
### 4.5.3.1 Acesso e configurações: transversal às três áreas
...
protocolo (`lotes_etapas`), que é dado de movimento e fica na §4.5.3.3.
...
## 4.5.5 Observação sobre a origem deste artefato

O modelo não foi desenhado antes da implementação nem extraído dela: as duas coisas andaram juntas.
O que está aqui é o modelo **especificado**, e o §4.5.2.1 declara o que dele existe no banco.
